...
const modules = {
  polymers: {
    ...
    title: "KINETIC LOAD DAMPING",
    desc: "Analyzing viscoelastic reaction under vertical stress. We measure kinetic energy dissipation to prevent material fatigue in footwear, mattresses, and ergonomic furniture.",
    spec: "DENSITY_CALIBRATION",
    specDesc: "Verifying molecular density. Sub-optimal density leads to rapid deformation in guest-facing assets under human weight loads.",
    type: "image", 
    // DİKKAT: Buraya senin indirdiğin yeni görselin adını yazıyoruz. 
    // Eğer görselin yoksa eski 'xray-slipper.jpg' yapabilirsin.
  ...
  fibers: {
    id: "02",
    label: "FIBER_ARCH", 
    title: "TENSILE STRENGTH MATRIX",
    desc: "Measuring abrasion resistance and fiber integrity. Critical for extending the lifecycle of high-stress items like staff uniforms and industrial-wash linens.",
    spec: "BREATHABILITY_INDEX",
    specDesc: "Calculating thermodynamic airflow through the weave to maximize comfort in guest textiles and staff apparel.",
    type: "viz_fiber",
  ...
  compounds: {
    id: "03",
    label: "BIO_COMPOUNDS", 
    title: "MOLECULAR SAFETY",
    desc: "Spectroscopic screening for hazardous volatile organic compounds (VOCs). Ensuring absolute dermatological safety in cosmetics and housekeeping chemicals.",
    spec: "DECOMPOSITION_RATE",
    specDesc: "Verifying the biodegradation timeline of all single-use amenities to ensure compliance with Global Green Star protocols.",
    type: "viz_compound",
...
export default function ExplodedView() {
  ...
  const data = modules[activeTab];

  return (
    <section className="min-h-screen border-y border-[#2A3241] bg-[#0B0E14] grid grid-cols-1 lg:grid-cols-2 overflow-hidden relative">
        ...
        <div className="flex gap-4 mb-12 border-b border-[#2A3241] pb-4 flex-wrap">
          {(Object.keys(modules) as Array<keyof typeof modules>).map((key) => (
            <button
              ...
              className={`font-mono text-xs tracking-widest px-4 py-2 border transition-all duration-300 ${
                activeTab === key 
          ...
          <div className="space-y-10">
            <div className="group">
              <div className="flex items-center gap-4 mb-2">
                ...
                <h3 className="font-mono text-lg text-white group-hover:text-[#00FF41] transition-colors">SCOPE_OF_ANALYSIS</h3>
              </div>
            ...
            <div className="group">
              <div className="flex items-center gap-4 mb-2">
                ...
                <h3 className="font-mono text-lg text-white group-hover:text-[#00FF41] transition-colors">{data.spec}</h3>
              </div>
              <p className="text-sm text-gray-400 pl-8 border-l border-gray-800 ml-2 leading-relaxed">
                {data.specDesc}
              </p>
      ...
      <div className="relative flex items-center justify-center bg-black min-h-[500px] overflow-hidden p-8 lg:p-16">
        ...
        <div className="relative z-10 w-full h-full max-w-[600px] max-h-[600px] flex items-center justify-center">
            <AnimatePresence mode="wait">
              ...
              > 
                 ...
                 {/* 2. FIBER (TEKSTİL ANİMASYONU) */}
                 {data.type === 'viz_fiber' && <FiberViz />}

                 {/* 3. COMPOUND (KİMYA ANİMASYONU) */}
        ...
        <div className="absolute bottom-6 right-6 font-mono text-[10px] text-[#00FF41] border border-[#00FF41]/30 px-4 py-2 bg-black/50 backdrop-blur-md z-30">
           {data.fig}
        </div>
      </div>
    </section>
