...
export default function ReportDetail() {
  ...
  // --- KRİTİK DÜZELTME ---
  // Linkten gelen ID (params.id) ile veritabanındaki ID'yi
  // ikisini de küçük harfe çevirerek karşılaştırıyoruz.
  // Böylece "RPT-7702-ANT" ile "rpt-7702-ant" aynı sayılır ve hata vermez.
  const report = reports.find((r) => 
    r.id.toLowerCase() === (params.id as string).toLowerCase()
  );

  if (!report) {
    return (
      <div className="min-h-screen bg-[#0B0E14] flex flex-col items-center justify-center text-gray-500 font-mono text-xs gap-4">
        <span>ERROR_404: REPORT_NOT_FOUND_ON_SERVER</span>
        <Link href="/archive" className="text-[#00FF41] border border-[#00FF41] px-4 py-2 hover:bg-[#00FF41] hover:text-black transition-all">
          RETURN TO ARCHIVE
        </Link>
      </div>
    );
  ...
  return (
    <main className="min-h-screen bg-[#0B0E14] text-white font-sans relative overflow-x-hidden selection:bg-[#00FF41] selection:text-black">
      ...
      <div className="max-w-4xl mx-auto p-6 md:p-12 relative">
        ...
        {/* TAM İÇERİK (HTML Olarak Basılıyor) */}
        ...
        <div className="relative mt-16 py-12 border-t border-[#2A3241] border-dashed">
           <div className="bg-[#151922] border border-[#2A3241] p-8 text-center max-w-lg mx-auto relative overflow-hidden group">
              ...
              {/* Mail Butonu: Tıklandığında ID ile birlikte mail taslağı açar */}
              <a 
